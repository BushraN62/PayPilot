...
COLLEGE_COLUMNS = [
    "id", "name", "state", "ownership", "degree", "tuition", "earnings",
    "debt", "graduation_rate", "acceptance_rate",
]
...
def _fetch_all(fields):
    """Fetch every API page safely; this is used only when a local cache is absent."""
    if not API_KEY:
        raise RuntimeError("COLLEGE_SCORECARD_API_KEY is required to build the local data cache.")
    ...
    while True:
        response = requests.get(
            BASE_URL,
            params={"api_key": API_KEY, "fields": ",".join(fields), "per_page": PER_PAGE, "page": page},
            timeout=30,
        )
...
def _download_programs():
    """Load field-of-study CIP data from Scorecard's separate bulk download."""
...
def _build_cache():
    ...
    colleges = [{
        ...
        "earnings": record.get("latest.earnings.10_yrs_after_entry.median"),
        "debt": record.get("latest.aid.median_debt.completers.overall"),
...
def search_colleges(
    ...
    """Search the local Scorecard cache, creating it once when needed."""
    if not COLLEGES_FILE.exists() or not PROGRAMS_FILE.exists():
        _build_cache()

    matching_ids = _matching_school_ids(major) if major else None
    selected_ownership = ownership_map.get(school_type) if school_type else None
    selected_degree = degree_map.get(degree_level) if degree_level else None
    colleges = []

    for row in _read_csv(COLLEGES_FILE):
        if state and row["state"] != state:
            continue
        if selected_ownership and _number(row["ownership"]) != selected_ownership:
            continue
        if selected_degree and _number(row["degree"]) != selected_degree:
            continue
        if min_earnings and (_number(row["earnings"]) is None or _number(row["earnings"]) < min_earnings):
            continue
        if max_debt and (_number(row["debt"]) is None or _number(row["debt"]) > max_debt):
            continue
        if max_tuition and (_number(row["tuition"]) is None or _number(row["tuition"]) > max_tuition):
            continue
        ...
        colleges.append({
            ...
            "tuition": _number(row["tuition"]), "earnings": _number(row["earnings"]),
            "debt": _number(row["debt"]), "graduation_rate": _number(row["graduation_rate"]),
